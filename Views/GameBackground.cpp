...
const std::string GameBackground::map_material{
    "GameMapG"}; //{"Examples/GrassFloor"};
const std::string GameBackground::skybox_material{
    "TD/StarSky"}; //{"Examples/CloudySky"};  //SpaceSkyBox"};
...
GameBackground::GameBackground(Ogre::SceneManager *scene, Ogre::Viewport *vport)
    : scene_mgmt(scene), root_node(scene_mgmt->getRootSceneNode()),
      view_port(vport) {
  // the image size to generate for the background. Will likely need to
  // dynamically resize this (and have a 4:3 aspect ratio?)
  bg_width = 1024;
  bg_height = 1024;

  // make the background generator
  bg_generator = std::make_unique<fflame_generator<data_t, pixel_t>>(
      bg_height, bg_width, 1);
  // pass in a queue to hold the finished flame frames
  bg_framequeue = std::unique_ptr<EventQueue<uint8_t[]>>(
      new EventQueue<uint8_t[]>(MAX_BGQUEUE));
...
void GameBackground::make_background() {
  ...
  for (size_t pt_idx = 0; pt_idx < map_extents.size(); ++pt_idx) {
    Ogre::Ray ray = cam->getCameraToViewportRay(map_extents.at(pt_idx)[0],
                                                map_extents.at(pt_idx)[1]);
    auto world_coord = ray.intersects(ray_aab);

    if (world_coord.first) {
      std::cout << "@ " << pt_idx << " --> " << ray.getPoint(world_coord.second)
                << std::endl;
    } else {
  ...
  std::cout << "map (@world coords) [" << map_height << ", " << map_width << "]"
            << std::endl;

  // the above doesn't really work too well, so just discard it and use the
  // extents below...
  ...
  Ogre::MeshManager::getSingleton().createPlane(
      map_planename, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
  ...
  */
  ...
  bg_texture = Ogre::TextureManager::getSingleton().createManual(
      "fflame_bgtexture",
      Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
      Ogre::TEX_TYPE_2D, bg_width, bg_height, 0, Ogre::PF_BYTE_RGBA,
      Ogre::TU_DYNAMIC_WRITE_ONLY_DISCARDABLE);
  bg_material = Ogre::MaterialManager::getSingleton().create(
      "fflame_bgmaterial",
      Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
  bg_material->getTechnique(0)->getPass(0)->createTextureUnitState(
      "fflame_bgtexture");
  bg_material->getTechnique(0)->getPass(0)->setSceneBlending(
  ...
  // load the default background -- acts as the placeholder until a new one is
  // generated
  Ogre::Image default_bg;
  default_bg.load("default_bg.png", "Popular");
  // this should be equivalent to default_bg.getData()?
  auto default_bgdatap =
      reinterpret_cast<uint8_t *>(default_bg.getPixelBox(0, 0).data);
  auto texture_buffer =
      std::unique_ptr<uint8_t[]>(new uint8_t[3 * bg_height * bg_width]);

  std::copy(default_bgdatap, default_bgdatap + 3 * bg_height * bg_width,
            texture_buffer.get());
  // std::fill(texture_buffer.get(), texture_buffer.get() + 3 * bg_height *
  // bg_width, 0);
  ...
  bg_node =
      scene_mgmt->getRootSceneNode()->createChildSceneNode("fflame_bgnode");
...
void GameBackground::render_background_texture(
    std::unique_ptr<uint8_t[]> texture_buffer) {
  ...
  for (int i = 0; i < bg_height; ++i) {
    ...
    // ogre doesnt guarentee the requested format, so we need to to this to have
    // the correct buffer stride
    bg_data += bg_pixelbox.getRowSkip() *
               Ogre::PixelUtil::getNumElemBytes(bg_pixelbox.format);
  }
...
void GameBackground::draw_background() {
  ...
  // update the material to use the new texture (this doesn't seem to happen
  // automatically)
  auto bg_texturestate =
      bg_material->getTechnique(0)->getPass(0)->getTextureUnitState(0);
...
void GameBackground::draw_tiles(const int num_cols, const int num_rows) {
  ...
  // TODO: we need to offset the lines to be drawn to the semi-transparent
  // middle rectangle
  ...
  for (size_t row = 0; row < map_grid_rowlines.size(); ++row) {
    ...
    map_grid_rowlines.at(row)->begin(mapgrid_material,
  ...
  for (size_t col = 0; col < map_grid_collines.size(); ++col) {
    ...
    map_grid_collines.at(col)->begin(mapgrid_material,
                                     Ogre::RenderOperation::OT_LINE_LIST);
    {
...
get_worldclick_coords(Ogre::SceneManager *scene_mgmt, Ogre::Viewport *view_port,
...
Ogre::MovableObject *user_select(Ogre::SceneManager *scene_mgmt,
                                 Ogre::Viewport *view_port, const float x,
                                 const float y) {
  auto cam = view_port->getCamera();
  ...
  if (!q_hits.empty()) {
    for (auto q_it = q_hits.begin(); q_it != q_hits.end(); ++q_it) {
      if (q_it->movable) {
        // NOTE: the difficulty here is that the ray will intersect with many
        // things in the scene. For now, we can assume we'll just want to click
        // on towers (and eventually, monsters). We will (never) want to click on
        // the GUI overlay in this manner (those inputs will be injected
        // elsewhere), and we don't want to click on the game map (AFAIK). So,
        // filter the results here to first return towers, then return monsters.
        // --> TODO: determine if there's a better way to select the appropriate
        // objects other than string comparisons?
...
// returns whether the click intersected the GameMap, and if so, what the
// distance was and a pointer to the object
std::tuple<bool, float, Ogre::MovableObject *>
check_point(Ogre::SceneManager *scene_mgmt, Ogre::Viewport *view_port,
            const float x, const float y) {
  ...
  if (verbose_print) {
    std::cout << "Viewport Actual Dim: [" << view_port->getActualLeft() << ", "
              << view_port->getActualTop() << " -- "
              << view_port->getActualHeight() << ", "
              << view_port->getActualWidth() << "]" << std::endl;
    std::cout << "Viewport Relative  Dim: [" << view_port->getLeft() << ", "
              << view_port->getTop() << " -- " << view_port->getHeight() << ", "
              << view_port->getWidth() << "]" << std::endl;
    std::cout << "Ray [" << x << ", " << y << "] origin: " << ray.getOrigin()
              << " @direction " << ray.getDirection() << std::endl;
    std::cout << "Ray @ near plane: "
              << ray.getPoint(cam->getNearClipDistance())
              << "Ray @ far plane: " << ray.getPoint(cam->getFarClipDistance())
              << std::endl;
    std::cout << "Ray (non-normalized) [" << x * view_port->getActualWidth()
              << ", " << y * view_port->getActualHeight() << "]" << std::endl;
  }

  std::tuple<bool, float, Ogre::MovableObject *> click_intersection{false, 0,
                                                                    nullptr};
  auto r_query = scene_mgmt->createRayQuery(ray);
  r_query->setSortByDistance(true);
  auto &q_hits = r_query->execute();
  if (!q_hits.empty()) {
    ...
    for (auto q_it = q_hits.begin(); q_it != q_hits.end(); ++q_it) {
      if (q_it->movable) {
        Ogre::MovableObject *obj = q_it->movable;
        std::cout << "moveable -- " << obj->getName() << ": "
                  << obj->getMovableType()
                  << " bounding: " << obj->getWorldBoundingBox() << std::endl;
        if (obj->getName() == GameBackground::map_name) {
    ...
    std::cout << "[" << x << ", " << y << "] Did not intersect with anything"
              << std::endl;
