...
void InputListener::windowClosed(Ogre::RenderWindow *window) {
  // do we need to do anything prior to the window being closed? -- this method
  // is called when the user clicks the window "x" button, but hasn't actually
  // been closed yet. hence, this is the place to trigger any cleanup methods
  // that require the renderwindow to still exist
...
bool InputListener::mouseMoved(const OIS::MouseEvent &mouse_arg) {
  ...
  if (mouse_dragging) {
    ...
    for (auto &m_listeners : input_listeners) {
      m_listeners.second->push(ControllerUtil::InputEvent(
          evt, ms.X.abs - drag_pos_x, ms.Y.abs - drag_pos_y));
    }
...
bool InputListener::mousePressed(const OIS::MouseEvent &mouse_arg,
...
bool InputListener::mouseReleased(const OIS::MouseEvent &mouse_arg,
                                  OIS::MouseButtonID mouse_id) {
  ControllerUtil::INPUT_TYPE evt;
  ...
  if (valid_input) {
    ...
    auto ms = mouse->getMouseState();
    for (auto &m_listeners : input_listeners) {
      m_listeners.second->push(
          ControllerUtil::InputEvent(evt, ms.X.abs, ms.Y.abs));
    }
  }
