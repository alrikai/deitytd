...
Controller::Controller(Ogre::Root *root, Ogre::RenderWindow *ogre_window)
    : window(ogre_window), ois_manager(nullptr), ois_mouse(nullptr),
      ois_keyboard(nullptr) {
  init();

  listener = std::unique_ptr<InputListener>(
      new InputListener(ois_mouse, ois_keyboard));
  ois_mouse->setEventCallback(listener.get());
  ois_keyboard->setEventCallback(listener.get());
  Ogre::WindowEventUtilities::addWindowEventListener(ogre_window,
                                                     listener.get());
...
void Controller::init() {
  ...
  ois_params.insert(
      std::make_pair(attribute_name, std::to_string(window_handle)));
  ...
  ois_keyboard = static_cast<OIS::Keyboard *>(
      ois_manager->createInputObject(OIS::OISKeyboard, true));
  ois_mouse = static_cast<OIS::Mouse *>(
      ois_manager->createInputObject(OIS::OISMouse, true));
}
...
bool Controller::register_input_listener(std::string id,
                                         ControllerBufferType *buffer) {
