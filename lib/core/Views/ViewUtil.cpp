/* ViewUtil.cpp -- part of the DietyTD Views subsystem implementation
 *
...
void nuke_movables(Ogre::SceneNode *t_scenenode) {
  ...
  while (child_scenenode_it.hasMoreElements()) {
    auto child_scenenode =
        static_cast<Ogre::SceneNode *>(child_scenenode_it.getNext());
...
void load_model(Ogre::SceneNode *t_scenenode,
                const CharacterModels::ModelIDs id,
                const std::string model_handleid) {
  /*
        const auto model_id =
     CharacterModels::id_names.at(static_cast<int>(id)); auto model_ent =
     t_scenenode->createEntity(model_handleid, model_id + ".mesh");
  ...
  */
  ...
  if(model_idit != CharacterModels::id_names.end()) {
    auto model_ent = t_scenenode->createEntity(model_handleid, *(model_id) +
  ".mesh"); model_ent->setRenderQueueGroup(Ogre::RENDER_QUEUE_MAIN);
    t_scenenode->attachObject(model_ent);
  } else {
