 ...
 * when taking cooridnates from the Views, they'll be normalized coordinate,
 * specifically real #'s between [0 ~ 1]. The Game Map will then map these
 * normalized coordinates to the correct tiles
 */
class GameMap {
public:
  ...
  GameMap() : tile_height(NormFactorHeight), tile_width(NormFactorWidth) {
    setmap_dims();
  }
  ...
  IndexCoordinate get_bounding_tile(const float col_location,
                                    const float row_location) const {
    // check boundary
    if (row_location > 1.0f || col_location > 1.0f || row_location < 0.0f ||
        col_location < 0.0f)
      return IndexCoordinate(-1, -1);

    // we want to get incides row e [0, MAP_HEIGHT), col e [0, MAP_WIDTH) from
    // the normalized input row | col vals
  ...
  IndexCoordinate get_bounding_tile(const Coordinate<float> &location) const {
    // check boundary
    if (location.row > 1.0f || location.col > 1.0f || location.row < 0.0f ||
        location.col < 0.0f)
      return IndexCoordinate(-1, -1);
  ...
  TowerCoordinate get_tower_block(const float col_location,
                                  const float row_location) const {
    // check boundary
    if (row_location > 1.0f || col_location > 1.0f || row_location < 0.0f ||
        col_location < 0.0f)
    ...
    const int tower_row = GameMap::TowerTileHeight *
                          (std::lround(std::floor(row_location / tile_height)) /
                           GameMap::TowerTileHeight);
    const int tower_col = GameMap::TowerTileWidth *
                          (std::lround(std::floor(col_location / tile_width)) /
                           GameMap::TowerTileWidth);

    // NOTE: returned range is [inclusive, exclusive)
    return TowerCoordinate(
        std::make_tuple(tower_col, tower_col + GameMap::TowerTileWidth),
  ...
  bool is_obstructed(const TowerCoordinate &tcoord) const {
    for (int map_r = std::get<0>(tcoord.row); map_r < std::get<1>(tcoord.row);
         ++map_r)
      for (int map_c = std::get<0>(tcoord.col); map_c < std::get<1>(tcoord.col);
           ++map_c)
  ...
  void set_obstructed(const float col_location, const float row_location,
                      bool obstruct_flag) {
    auto map_coord = get_bounding_tile(col_location, row_location);
  ...
  Coordinate<double> get_tile_center(const float col_location,
                                     const float row_location) const {
    auto map_coord = get_bounding_tile(col_location, row_location);
  ...
  Coordinate<double> get_block_center(const TowerCoordinate &tcoord) const {
    double block_center_row =
        std::get<0>(tcoord.row) +
        (std::get<1>(tcoord.row) - std::get<0>(tcoord.row)) / 2.0;
    double block_center_col =
        (std::get<0>(tcoord.col) +
         (std::get<1>(tcoord.col) - std::get<0>(tcoord.col)) / 2.0);
    return Coordinate<double>(tile_width * block_center_col,
                              tile_height * block_center_row);
  }
  ...
  void remove_mob(Coordinate<float> tile_coord, const MobID &mob_id) {
    ...
    auto mob_it = std::find_if(target_tile->resident_mobs.begin(),
                               target_tile->resident_mobs.end(),
                               [mob_id](const std::weak_ptr<Monster> &m) {
                                 // TODO: we might want to use different
                                 // comparison schemes for identifying a mob
                                 // (comparing strings is slower than say, ints)
                                 // will want to move this kind of thing into
                                 // the Monster class itself
    ...
    if (mob_it != target_tile->resident_mobs.end()) {
      if (auto target_mob = mob_it->lock()) {
        std::cout << "removing mob " << target_mob->get_name() << " from tile ("
                  << tile_idx.col << ", " << tile_idx.row << ")" << std::endl;
      }
      target_tile->resident_mobs.erase(mob_it);

      std::cout << "Target tile @(" << tile_idx.col << ", " << tile_idx.row
                << ")"
                << " has " << target_tile->resident_mobs.size() << " #mobs left"
                << std::endl;
    } else {
    ...
    /*
    std::remove_if(target_tile->resident_mobs.begin(),
    target_tile->resident_mobs.end(), [mob_id](const std::weak_ptr<Monster> &m)
      {
        //TODO: we might want to use different comparison schemes for
    identifying a mob (comparing strings is slower than say, ints)
  ...
  void setmap_dims() {
    for (int tile_row = 0; tile_row < MAP_HEIGHT; ++tile_row) {
      for (int tile_col = 0; tile_col < MAP_WIDTH; ++tile_col) {
        ...
        map[tile_row][tile_col].tile_center.row =
            tile_height * (tile_row + 0.5);
      }
    }
  }
