...
std::tuple<std::vector<std::vector<uint32_t>>, std::vector<std::vector<float>>> make_mesh()
{
	...
	std::vector<cv::Mat_<PixelType>> image_stack;
	for (int z = 0; z < imdepth; ++z)
	{
	    cv::Mat_<PixelType> img = cv::Mat_<PixelType>::zeros(imheight, imwidth);
        img(roi) = 1;

		image_stack.push_back(img);
	}

    for (int depth = 0; depth < imdepth; ++depth)
    {
        auto image = image_stack.at(depth);
        for (int row = 0; row < imheight; ++row)
        {
            for (int col = 0; col < imwidth; ++col)
            {
                if(image(row,col) > 0)
                    pt_cloud->push_back(pcl::PointXYZ(col,row,depth));
            }
        }
    }
    */
