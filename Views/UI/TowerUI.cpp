...
void TowerUpgradeUI::initialize_wordcomboUI() {
  gui_wordcombine_window =
      CEGUI::WindowManager::getSingleton().loadLayoutFromFile(
          "DTDWordCombineUI.layout");
  // gui_wordcombine_window->setParent(gui_window);
  gui_wordcombine_window->setVisible(false);

  gui_wordcombine_layout = reinterpret_cast<CEGUI::HorizontalLayoutContainer *>(
      gui_wordcombine_window->getChild("WordCombineBackground"));

  auto word_combine_window =
  ...
  word_combine_window->getChild("PreviewButton")
      ->subscribeEvent(CEGUI::PushButton::EventClicked,
                       CEGUI::Event::Subscriber(
                           &TowerUpgradeUI::wordcombine_previewbtn, this));
  word_combine_window->getChild("CombineButton")
      ->subscribeEvent(CEGUI::PushButton::EventClicked,
                       CEGUI::Event::Subscriber(
                           &TowerUpgradeUI::wordcombine_combinebtn, this));
  word_combine_window->getChild("ClearButton")
      ->subscribeEvent(CEGUI::PushButton::EventClicked,
                       CEGUI::Event::Subscriber(
                           &TowerUpgradeUI::wordcombine_clearbtn, this));
  word_combine_window->getChild("CancelButton")
      ->subscribeEvent(CEGUI::PushButton::EventClicked,
                       CEGUI::Event::Subscriber(
                           &TowerUpgradeUI::wordcombine_cancelbtn, this));
  ...
  // NOTE: you can make this more robust by getting the position and size, then
  // placing the inventory after it. it works as it currently is though auto
  // wcombo_position =
  // gui_wordcombine_layout->getPositionOfChild(word_combine_window); std::cout
  // << "word combine panel position: " << wcombo_position << std::endl;
  gui_wordcombine_layout->addChild(gui_inventory_window);
  gui_inventory_window->setPosition(
      CEGUI::UVector2(CEGUI::UDim(0.6, 0), CEGUI::UDim(0, 0)));
  ...
  for (int row = 0; row < inventory_rows; row++) {
    for (int col = 0; col < inventory_cols; col++) {
      ...
      auto target_inventory_slot =
          gui_inventory_window->getChild("InventoryPanel")->getChild(slot_id);

      // for testing: enable the slot. Also note that you should have the
      // background be non-draggable, and only enable dragging capabilities if
      // there's an item in that inventory slot.
      auto slot_dragger = reinterpret_cast<CEGUI::DragContainer *>(
          target_inventory_slot->getChild("dragger"));
      /*
                  slot_dragger->subscribeEvent(CEGUI::DragContainer::EventDragEnded,
                          CEGUI::Event::Subscriber(&TowerUpgradeUI::handle_inventory_item_dropped,
         this));
      */
      slot_dragger->subscribeEvent(
          CEGUI::DragContainer::EventDragStarted,
          CEGUI::Event::Subscriber(
              &TowerUpgradeUI::handle_inventory_item_dragging, this));

      // by default we don't want to have the backgrounds be draggable
      slot_dragger->setDraggingEnabled(false);
      // NOTE: the ID's here will overlap with the IDs in the word combo slots.
      // This shouldn't (?) be an issue, but we'll have to be aware of it..
      slot_dragger->setID(inventory_idx);
      inventory_idx += 1;
    }
  }

  // load the imageset for handling the letter tiles -- Q: do we even need to do
  // this?
  // CEGUI::ImagesetManager::getSingleton().createImageset("DTDLetters.imageset");
  // letter_imgset =
  // CEGUI::ImagesetManager::getSingleton().getImageset("DTDLetters");
  ...
  // Initially, (i.e. when we first make this window), we just want to have
  // EVERYTHING deactivated, since there will be no towers or anything. (and it
  // wont even be visible)
  // TODO: this is a completely arbitrary # (is just the size of the inventory).
  // Should make this the longest word in the dictionary, and make the inventory
  // larger
  const int max_num_letters = 20;
  auto word_panel = gui_wordcombine_layout->getChild("DTDWordCombinePanel")
                        ->getChild("WordDropperPanel");
  for (int letter_slotidx = 0; letter_slotidx < max_num_letters;
       letter_slotidx++) {
    ...
    auto slot_dragger = reinterpret_cast<CEGUI::DragContainer *>(
        target_letter_slot->getChild("ldragger"));
    slot_dragger->setDraggingEnabled(false);
    slot_dragger->setID(letter_slotidx);

    slot_dragger->subscribeEvent(
        CEGUI::Window::EventDragDropItemDropped,
        CEGUI::Event::Subscriber(&TowerUpgradeUI::handle_letter_item_dropped,
                                 this));
...
// Q: how best to handle the inventory updating? We would need to have some sort
// of 'previous' inventory state, and just change parts of it? Or just wholesale
// re-create the inventory each time?
  ...
  for (int row = 0; row < PlayerInventory::NUM_INVENTORY_ROWS; row++) {
    for (int col = 0; col < PlayerInventory::NUM_INVENTORY_COLS; col++) {
      ...
      if (inventory->inventory_occupied[inventory_idx]) {
        ...
        // <<
        // inventory->inventory_data[row*PlayerInventory::NUM_INVENTORY_COLS+col].letter
        // << std::endl;
        ...
        // Q: can we just set the text? on the staticimage? or do we need to
        // have an imageset of letters?
        auto target_letter = inventory->inventory_data[inventory_idx].letter;
        std::transform(target_letter.begin(), target_letter.end(),
                       target_letter.begin(), ::toupper);
...
// TODO: need to figure out a good way to manage the inventory. How to handle
// swapping items, reorganizing them, and maintaining the state between the
// player inventory and how the user re-arranges the items in the UI (i.e. how to
// keep track of both)
bool TowerUpgradeUI::handle_inventory_item_dropped(
  ...
  std::cout << "@inventory dragged handler -- window #child: "
            << dd_args.window->getChildCount()
            << " id: " << dd_args.window->getID() << std::endl;
  std::cout << "@inventory dragged handler -- droppeditem #child: "
            << dd_args.dragDropItem->getChildCount()
            << " id: " << dd_args.dragDropItem->getID() << std::endl;
...
bool TowerUpgradeUI::handle_letter_item_dropped(const CEGUI::EventArgs &args) {
  ...
  // Q: can we just set the text? on the staticimage? or do we need to have an
  // imageset of letters?
  ...
  // TODO: need to also update the inventory state... how to handle cacelling
  // though?
  //
...
bool TowerUpgradeUI::word_combination_evthandler(const CEGUI::EventArgs &e) {
  ...
  gui_wordcombine_layout->getChild("DTDWordCombinePanel")
      ->getChild("CurrentStatsEdit")
      ->setText(tower_info_str);
  gui_wordcombine_layout->getChild("DTDWordCombinePanel")
      ->getChild("CombinedStatsEdit")
      ->setText(tower_info_str);

  // TODO: load the tower image here as well... this will have to be a dynamic
  // image, as we are procedurally generating the tower models. Should be
  // something where we take a 2D snapshot of the tower's 3D model when we
  // generate it, and store it in a buffer. Then the first time we display it, we
  // need to make a Ogre/CEGUI texture and write the buffer data to it (and
  // finally, display it when we get to here)
  //...
  ...
  std::cout << "activating the first " << num_word_slots << " word slots"
            << std::endl;

  // this is to enable the word slot again -- so we can drag letters to enabled
  // slots
  ...
  // NOTE: we will have ALL N letter slots present in the UI, but only M <= N
  // slots will be active, based on the tower itself (and maybe the player? i.e.
  // upgrade to make higher #slots?)
  for (int letter_slotidx = 0; letter_slotidx < num_word_slots;
       letter_slotidx++) {
    std::string slot_id = "letter_" + std::to_string(letter_slotidx);
    auto target_letter_slot = word_panel->getChild(slot_id);
    ...
    // we never want to be able to drag FROM these slots (I think...)
    // auto slot_dragger =
    // reinterpret_cast<CEGUI::DragContainer*>(target_letter_slot->getChild("ldragger"));
    // slot_dragger->setDraggingEnabled(false);

    enable_letter_slot(target_letter_slot);
  }

  // NOTE: we assume that the inventory will have already been updated with the
  // player state, since that happens on every iteration of the frontend (so we
  // don't need to explicitly populate the player inventory)
