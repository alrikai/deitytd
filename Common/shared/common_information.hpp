...
/*
 * the idea is to have something here that'll be written to by the backend and
 * read by the frontend, which will be threadsafe and will allow for on-demand
 * information to be obtained by the frontend (rather than going through the
 * usual asynch-queue communication approach).
 */
...
public:
  ...
  void add_new_towerinfo(tower_key_t tower_id, tower_info_t info) {
    ...
    if (!t_ins.second) {
      ...
      std::cout << "ERROR -- insertion of ID " << tower_id
                << " was unsuccessful" << std::endl;
      std::string error_str{"ERROR -- ID " + std::to_string(tower_id) +
                            " exists already"};
