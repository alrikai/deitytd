...
{
    modifier_cfg.crit_multiplier_percent = mod_attributes["value_critmult"].as<float>(); 
    modifier_cfg.scale_factor = mod_attributes["scale_factor"].as<float>();
}

void parse_modifier_parameters(const YAML::Node& mod_attributes, flat_type_damage::parameter_cfg& modifier_cfg)
{
    modifier_cfg.type = static_cast<Elements>(mod_attributes["dmg_type"].as<int>()); 
...
/*
void parse_modifier_parameters(const YAML::Node& mod_attributes, ::parameter_cfg& modifier_cfg)
{
}
