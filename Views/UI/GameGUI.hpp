...
/*
//TODO: consolidate this and the OgreDisplay so we only have 1 framelistener on
the View side of things class UIFrameListener : public Ogre::FrameListener,
public Ogre::WindowEventListener
{
public:

    UIFrameListener (Ogre::RenderWindow* rwin, Ogre::Camera* cam,
...
/*
 * NOTE: this has to be initialized after the OgreDisplay is initialized (since
 * CEGUI needs an Ogre RenderWindow + general setup stuff to get started)
 */
class GameGUI {
public:
  ...
  // Q: how involved do we want the inputs to be? i.e. I could register GUI
  // handlers for key down
  // and key up events -- at the moment I'm just doing key down, but that
  // doesn't seem sufficient in the long run.
  template <class Controller> void register_controller(Controller *controller) {
    // this is where we set up the GUI event handlers
    std::function<void(OIS::KeyEvent, bool)> gui_keyhandler =
        [](OIS::KeyEvent key_arg, bool key_pressed) {
    ...
    std::function<void(OIS::MouseEvent, OIS::MouseButtonID, int)>
        gui_mousehandler = [cvt_mouseevts](OIS::MouseEvent mouse_evt,
                                           OIS::MouseButtonID mouse_id,
                                           int mouse_state) {
          // std::cout << "mouse GUI event handler" << std::endl;
          auto &context = CEGUI::System::getSingleton().getDefaultGUIContext();
          // context.injectMouseMove(mouse_evt.state.X.rel,
          // mouse_evt.state.Y.rel);
          context.injectMousePosition(mouse_evt.state.X.abs,
                                      mouse_evt.state.Y.abs);
    ...
    controller->register_gui_listener(std::move(gui_keyhandler),
                                      std::move(gui_mousehandler));
  }

  void register_shared_towerinfo(
      std::shared_ptr<
  ...
  //--------------------------------------------------------------------------------------------------------------------------------------
  // NOTE: these are sort of redundant, as we now have the shared_gamestate_info
  // as a member variable. So, we don't need to have external input regarding the
  // player info updates. How to handle this?
  inline void update_inventory_info(const TDPlayerInformation &info) {
    // update the inventory state based on the new player snapshot when the
    // round ends and the
  ...
  inline std::pair<bool, TDPlayerInformation> get_inventory_updates() const {
    ...
    //.... so far the only place that would happen is in the tower UI. In theory
    //we will eventually add more, different UIs which could also affect the
    // player inventory. In which case, we'll have to aggregate the inventory
    // changes and keep a consistent version.... somewhere, s.t. when a UI opens
    // it'll get the current inventory, and when it closes we'll update our
    // frontend master copy of the inventory with any changes that may have been
    // made. Or something like that

    auto ui_snapshot = tower_modify_ui->get_inventory_updates();

    // TODO: this is where we would do some sort of check to see if the
  ...
  // called by the main frontend class in response to user input. This is where
  // we make the TowerUpgrade UI the active window
  void handle_tower_upgrade(uint32_t active_tower_ID) {
    tower_modify_ui->activate_towerUI(active_tower_ID);
  }

  void display_information(const std::string &base_stats,
                           const std::string &current_stats,
                           const std::string &unit_info);

  // returns whether a subwindow is showing. Is used to disable certain things
  // in the frontend (i.e. panning the camera)
  inline bool is_subwindow_showing() const {
    // TODO: eventually (as we add other UI windows) there would be others to
    // check
  ...
  // kind of weird to return a pointer to a vector... maybe I should just return
  // an iterator? I want to modify the TowerUI vector however. or... I could just
  // return a copy of the vector, and clear the one in the TowerUI? this assumes
  // that the caller will handle all of the events... but that seems like a safe
  // bet anyways
  inline std::vector<TowerUpgradeUI::tower_modifyinfo>
  get_pending_tower_modifications() const {
