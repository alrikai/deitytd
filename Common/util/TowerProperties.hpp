...
struct tower_property_modifier
{
    ...
    tower_property_modifier()
      : damage_value{{dmg_dist(0,0),dmg_dist(0,0),dmg_dist(0,0),dmg_dist(0,0),dmg_dist(0,0)}}
    {
    ...
    //event lists for on-hit and on-death effects. When these events are triggered, will loop
    //through these event lists and fire them off
...
struct tower_properties
{
    ...
    tower_properties()
        : damage{{dmg_dist(0,0),dmg_dist(0,0),dmg_dist(0,0),dmg_dist(0,0),dmg_dist(0,0)}}
    {
        attack_speed = 0.f;
        attack_range = 0.f;
    ...
    tower_properties& operator +=(const tower_properties& rhs_modifier)
    {
        ...
        on_hit_events.insert(std::end(on_hit_events), std::begin(rhs_modifier.on_hit_events), std::end(rhs_modifier.on_hit_events));
        on_death_events.insert(std::end(on_death_events), std::begin(rhs_modifier.on_death_events), std::end(rhs_modifier.on_death_events));

        return *this;
    }

    //NOTE: there's an optimization oppotunity here to move from the modifier's on-event vectors?
    void apply_property_modifier(tower_property_modifier modifier)
    {
        ...
        for (int dmg_idx = 0; dmg_idx < NUM_ELEM; dmg_idx++) {
            ...
            damage[dmg_idx] += damage[dmg_idx] * modifier.enhanced_damage_value[dmg_idx];
        }
        ...
        on_hit_events.insert(std::end(on_hit_events), std::begin(modifier.on_hit_events), std::end(modifier.on_hit_events));
        on_death_events.insert(std::end(on_death_events), std::begin(modifier.on_death_events), std::end(modifier.on_death_events));
    }
