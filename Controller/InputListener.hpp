...
public:
  InputListener(OIS::Mouse *ois_mouse, OIS::Keyboard *ois_keyboard)
      : mouse(ois_mouse), keyboard(ois_keyboard), frame_count(0),
        current_key(OIS::KeyCode::KC_UNASSIGNED),
        current_mouse(OIS::MouseButtonID::MB_Button7) {
    ...
    gui_mouse_evthandler = [](OIS::MouseEvent, OIS::MouseButtonID mouse_id,
                              int mouse_flag) {};
  }
  ...
  bool mousePressed(const OIS::MouseEvent &mouse_arg,
                    OIS::MouseButtonID mouse_id) override;
  bool mouseReleased(const OIS::MouseEvent &mouse_arg,
                     OIS::MouseButtonID mouse_id) override;

  void window_resized(Ogre::RenderTarget *ogre_window);

  inline bool add_input_listener(std::string id,
                                 ControllerUtil::ControllerBufferType *buffer) {
    return input_listeners.insert(std::make_pair(id, buffer)).second;
    ;
  }

  inline void register_gui_handler(
...
private:
  std::unordered_map<std::string, ControllerUtil::ControllerBufferType *>
      input_listeners;
  ...
  // initialize it to be empty -- will be registered later after the GUI is set
  // up
  std::function<void(OIS::KeyEvent key_arg, bool)> gui_key_evthandler;
  std::function<void(OIS::MouseEvent mouse_arg, OIS::MouseButtonID mouse_id,
                     int)>
      gui_mouse_evthandler;

  // eventually we might want to support having multiple keys pressed (e.g. alt
  // + tab or something)
