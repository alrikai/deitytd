...
template <typename pixel_t> using frame_t = FFlames::flame_frame<pixel_t>;

template <typename data_t, typename pixel_t> class fflame_generator {
public:
  fflame_generator(const int imheight, const int imwidth,
                   const int num_workers = std::thread::hardware_concurrency())
      : num_workers(num_workers), imheight(imheight), imwidth(imwidth),
        fflame_histoqueue(100, 30), flame_prebarrier(num_workers),
        flame_postbarrier(num_workers), fflame_th(nullptr),
        fflame_histdata(nullptr),
        total_variant_rng(
            0,
            FFlames::affine_fcns::variant_list<data_t>::variant_names.size() -
                1) {
    ...
    ff_renderer =
        std::make_unique<FFlames::fflame_renderer<data_t>>(imheight, imwidth);
...
private:
  void initialize_variants() {
    // std::vector<std::shared_ptr<affine_fcns::variant<data_t>>>
    // working_variants(num_working_variants);
    std::vector<std::string> working_variants(num_working_variants);
    working_variants[0] = "linear";

    for (int i = 1; i < num_working_variants; ++i) {
      working_variants.at(i) = FFlames::affine_fcns::variant_list<
          data_t>::variant_names[total_variant_rng(flame_gen)];
      // auto selected_variant =
      // affine_fcns::variant_list<data_t>::variant_names[total_variant_rng(flame_gen)];
      // working_variants.at(i) =
      // std::shared_ptr<affine_fcns::variant<data_t>>(variant_maker.flame_maker.create_product(selected_variant));
    }

    flamer = std::make_unique<FFlames::affine_fcns::invoker<data_t>>(
        std::move(working_variants));
...
void fflame_generator<data_t, pixel_t>::start_fflame_generation() {
  ...
  for (int th_idx = 0; th_idx < num_workers; ++th_idx) {
    fflame_workers.emplace_back(new FFlames::flame_thread(
        flame_dist(flame_gen), flame_dist(flame_gen)));
...
FFlames::histogram_info<pixel_t>
print_mat(const cv::Mat_<FFlames::histogram_info<pixel_t>> &hdata, int row,
          int col) {
...
// threads
template <typename data_t, typename pixel_t>
void fflame_generator<data_t, pixel_t>::generate_fflame(
    FFlames::fflame_util::fast_rand &rand_gen) {
  ...
  while (fflame_state.load()) {
    ...
    FFlames::run_fflame<data_t, pixel_t>(
        flamer.get(), FFlames::fflame_constants::num_samples / num_workers,
        fflame_histdata.get(), rand_gen);

    // 2. wait for all the threads to finish the previous round. have 1 thread
    // execute the following steps:
    flame_prebarrier.wait();
    if (std::this_thread::get_id() == worker_overloard_id) {
      // somewhat unfortunate, but need to have this on the heap to avoid
      // scoping problems
      auto hist_info =
          std::make_unique<std::vector<FFlames::histogram_info<pixel_t>>>(
              imheight * imwidth);
      ...
      auto selected_variant = FFlames::affine_fcns::variant_list<
          data_t>::variant_names[total_variant_rng(flame_gen)];
      // replace a random variant (that's not the linear variant)
      int mod_idx =
          total_variant_rng(flame_gen) % (num_working_variants - 1) + 1;
      flamer->fcn.at(mod_idx).reset(
          variant_maker.flame_maker.create_product(selected_variant));
      flamer->randomize_parameters(-2, 2);
...
void fflame_generator<data_t, pixel_t>::render_fflame() {
  ...
  while (fflame_state.load()) {
    ...
    if (got_histdata && hist_info) {
      ...
      image = frame_t<pixel_t>(FFlames::fflame_constants::imheight,
                               FFlames::fflame_constants::imwidth,
                               pixel_t(0, 0, 0));
...
continue;
      ...
      */
      ...
      const std::string raw_impath =
          "TDraw_image_" + std::to_string(raw_counter++) + ".png";
      ...
      std::unique_ptr<uint8_t[]> im_data = std::unique_ptr<uint8_t[]>(
          new uint8_t[3 * FFlames::fflame_constants::imheight *
                      FFlames::fflame_constants::imwidth]);
      int im_data_idx = 0;
