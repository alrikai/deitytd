...
void GameGUI::set_lives(int amount) {
  ...
  auto new_string =
      amount > 0 ? lives_string + std::to_string(amount) : "You're Dead!";
...
void GameGUI::setup_animations() {
  CEGUI::AnimationManager &animation_mgmt =
      CEGUI::AnimationManager::getSingleton();
  animation_mgmt.loadAnimationsFromXML("DTDLook.anims");

  CEGUI::Animation *right_spinning_anim =
      animation_mgmt.getAnimation("RotateRightLoop");
  CEGUI::Animation *left_spinning_anim =
      animation_mgmt.getAnimation("RotateLeftLoop");

  CEGUI::AnimationInstance *menu_anim =
      animation_mgmt.instantiateAnimation(right_spinning_anim);
  CEGUI::Window *menu_button = gui_window->getChild("menu_button");
  menu_anim->setTargetWindow(menu_button);
  menu_anim->start();

  CEGUI::AnimationInstance *quit_anim =
      animation_mgmt.instantiateAnimation(left_spinning_anim);
...
void GameGUI::initialize_mainUI() {
  // this call sets up all the CEGUI system objects with Ogre3D defaults.
  // Namely, This will create and initialise the following objects for you:
  //- CEGUI::OgreRenderer
  //- CEGUI::OgreResourceProvider
  //- CEGUI::OgreImageCodec
  //- CEGUI::System
  // Can get the created objects later via singletons, e.g.
  ...
  CEGUI::System::getSingleton()
      ...
      .getMouseCursor()
      .setDefaultImage("DTDLook/MouseArrow");
  gui_window = CEGUI::WindowManager::getSingleton().loadLayoutFromFile(
      "DTDMainUI.layout");

  // TODO: this should spawn the main menu
  auto menu_button_clicked = [](const CEGUI::EventArgs &e) {
    std::cout << "NOTE: menu button was clicked, but nothing will happen"
              << std::endl;
    return true;
  };

  // TODO: should move this out to another function that'll trigger the shutdown
  // sequence
  auto quit_button_clicked = [](const CEGUI::EventArgs &e) {
    std::cout << "NOTE: quit button was clicked, but nothing will happen"
              << std::endl;
    return true;
  };
...
void GameGUI::display_information(const std::string &base_stats,
                                  const std::string &current_stats,
                                  const std::string &unit_info) {
  // TODO: should colorize the current stats, maybre re-arrange the fields a
  // bit?
  gui_window->getChild("staticinfo_text")
      ->getChild("stats_info")
      ->setText(base_stats);
  gui_window->getChild("staticinfo_text")
      ->getChild("stats_info")
      ->insertText(current_stats, base_stats.size() + 1);
  gui_window->getChild("staticinfo_text")
      ->getChild("description_info")
      ->setText(unit_info);
}
