...
std::ostream &operator<<(std::ostream &out_stream,
  ...
  for (int eidx = 0; eidx < tower_properties::NUM_ELEM; eidx++) {
    out_stream << element_names[eidx] << " Damage: [" << props.damage[eidx].low
               << ", " << props.damage[eidx].high << "] \n";
  }
  out_stream << "Speed: " << props.attack_speed
             << " Range: " << props.attack_range << "\n";
  out_stream << "Crit: " << props.crit_chance
             << " Crit Multiplier: " << props.crit_multiplier;
  return out_stream;
}
//----------------------------------------------------------------------------------------
