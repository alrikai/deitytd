...
struct MoveableObject {
  ...
  virtual void update_movedest(const Ogre::Vector3 &dest_pos,
    ...
    /*
    /////////////////////////////////////////////////////////////////////////////////////////
    //for testing: seeif we have a situation where the destination is further
    than the current pos.
    ...
    //get the distance from the final destination. In theory, the
    move_destination should be closer than the current position float src_dist =
    std::sqrt((full_dest.x - attack_pos.x) * (full_dest.x - attack_pos.x) +
                               (full_dest.y - attack_pos.y) * (full_dest.y -
    attack_pos.y)); float dest_dist = std::sqrt((full_dest.x - dest_fullpos.x) *
    (full_dest.x - dest_fullpos.x) + (full_dest.y - dest_fullpos.y) *
    (full_dest.y - dest_fullpos.y));
    */
  }
  ...
  virtual void update_position(const float time_elapsed) {
    if (is_active) {
      ...
      // std::cout << origin_id << " -- time delta: " << move_duration <<
      // std::endl;
      is_active = (move_duration >= 0);
      if (!is_active)
        std::cout << "Update Position Turning off -- over time limit!"
                  << std::endl;
    }
  }
...
template <class BackendType> struct TDMapInformation {
  ...
  // TODO: need to see which coordinated are best/most convenient for storage
  // purposes here...
  void add_tower_ID(float xnorm_coord, float ynorm_coord,
                    const uint32_t tower_ID) {
    ...
    // TODO: not really sure what to do if this ever happens... might be
    // exception throwin' time
    if (tower_ID_mapping[tile_row][tile_col]) {
      std::cerr << "ERROR -- TDMapInformation already has ID "
                << tower_ID_mapping[tile_row][tile_col] << " at " << tile_row
                << ", " << tile_col << std::endl;
    }
    ...
    if (id_it != location_mapping.end()) {
      // update the location... or throw an error, not sure if towers should be
      // movable?
      id_it->second = std::make_pair(tile_row, tile_col);
    } else {
      location_mapping.insert(
          std::make_pair(tower_ID, std::make_pair(ynorm_coord, xnorm_coord)));
    }
  }
  ...
  uint32_t get_tower_ID(float xnorm_coord, float ynorm_coord) {
    ...
    // map the normalized coordinates to the map tile (Q: will there be rounding
    // issues here?)
    float xtile = xnorm_coord * BackendType::MAP_NUMTILES_WIDTH;
    float ytile = ynorm_coord * BackendType::MAP_NUMTILES_HEIGHT;
    int tile_col = std::floor(xtile);
  ...
  std::tuple<bool, float, float> get_tower_location(const uint32_t tower_ID) {
    ...
    if (id_it != location_mapping.end()) {
      ...
      // NOTE: this might be indicative of a logic error, as we shouldn't have
      // scenrios where the ID is not a valid one
      std::cout << "ERROR -- tower ID " << tower_ID
                << " does not correspond to a valid tower" << std::endl;
      return std::make_tuple(false, -1.f, -1.f);
    }
  }

  // for faster lookup between user clicks and getting the constrituent tower ID
  static constexpr double UNITS_PER_TILE_H =
      1.0 / BackendType::MAP_NUMTILES_HEIGHT;
  static constexpr double UNITS_PER_TILE_W =
      1.0 / BackendType::MAP_NUMTILES_WIDTH;
  using TowerMapContainer =
      std::array<std::array<uint32_t, BackendType::MAP_NUMTILES_WIDTH>,
                 BackendType::MAP_NUMTILES_HEIGHT>;
...
class OgreDisplay : public Ogre::FrameListener,
...
public:
  ...
  */
  OgreDisplay()
    ...
    // not really sure if these should get their own method, or be rolled into
    // an existing one. So for now, just have them here...
    ...
    input_events = std::unique_ptr<ControllerUtil::ControllerBufferType>(
        new ControllerUtil::ControllerBufferType());
  ...
  void place_tower(TowerModel *selected_tower, const uint32_t tower_ID,
                   const std::string &tower_name,
                   const Ogre::AxisAlignedBox &map_box,
                   Ogre::Vector3 map_coord_offsets,
                   Ogre::Vector3 world_coord_offsets);
  void place_mob(const CharacterModels::ModelIDs id,
  ...
  Ogre::Root *get_root() const { return root.get(); }
  Ogre::RenderWindow *get_render_window() const { return render_window; }
  Ogre::SceneManager *get_scene_mgmt() const { return scene_mgmt; }
...
private:
  ...
  void get_mapcoords(const std::vector<float> &world_position, float &x_coord,
                     float &y_coord, const Ogre::AxisAlignedBox &map_box);
  void generate_tower(const float x_coord, const float y_coord,
                      const float click_distance,
                      const Ogre::AxisAlignedBox &mapobj);
  ...
  // the coefficients for camera movement (there will be more as we add more
  // functionality)
  ...
  // the tower event queues, for frontend --> backend and backend --> frontend
  // communication
  ...
  // holds the state of the current selection of the user (or nullptr if none
  // selected) this will presumably either be towers or mobs -- wouldn't make
  // sense to be able to click attacks
...
template <class BackendType>
const std::string OgreDisplay<BackendType>::resource_cfg_filename{
    "resources.cfg"};
...
template <class BackendType> void OgreDisplay<BackendType>::start_display() {

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////
  auto tbuild_evt_fcn =
      [this](std::unique_ptr<RenderEvents::create_tower> render_evt) {
        Ogre::Vector3 t_map_offsets{render_evt->t_map_offsets[0],
                                    render_evt->t_map_offsets[1],
                                    render_evt->t_map_offsets[2]};
        Ogre::Vector3 t_world_offsets{render_evt->t_world_offsets[0],
                                      render_evt->t_world_offsets[1],
                                      render_evt->t_world_offsets[2]};

        // NOTE: this is the last piece of the puzzle. Should be the GameMap
        // (GameBGMap), but I formally would get it from a scene query.
        Ogre::AxisAlignedBox map_box = this->background->get_map_aab();
        this->place_tower(render_evt->t_model.get(), render_evt->t_ID,
                          render_evt->t_name, map_box, t_map_offsets,
                          t_world_offsets);
      };

  auto atkbuild_evt_fcn =
      [this](std::unique_ptr<RenderEvents::create_attack> render_evt) {
        auto origin_tname = render_evt->origin_tname;
        Ogre::Vector3 origin = scene_mgmt->getEntity(origin_tname)
                                   ->getParentSceneNode()
                                   ->getPosition();
        Ogre::Entity *tower_atk = scene_mgmt->createEntity(
            render_evt->name, Ogre::SceneManager::PT_SPHERE);
  ...
  auto atkmove_evt_fcn =
      [this](std::unique_ptr<RenderEvents::move_attack> render_evt) {
        ...
        if (tower_it != tower_attacks.end()) {
          ...
          auto parent_snode =
              scene_mgmt->getEntity(attack_id)->getParentSceneNode();
          Ogre::Vector3 movement{render_evt->delta[0], render_evt->delta[1],
                                 parent_snode->getPosition().z};
          // the position for the attack node to be at after the time_duration
          // elapses
  ...
  auto atkremove_evt_fcn =
      [this](std::unique_ptr<RenderEvents::remove_attack> render_evt) {
        auto attack_id = render_evt->name;
        auto tower_it = tower_attacks.find(attack_id);
        if (tower_it != tower_attacks.end())
          tower_attacks.erase(tower_it);

        Ogre::SceneNode *t_scenenode =
            scene_mgmt->getEntity(attack_id)->getParentSceneNode();
        OgreUtil::nuke_scenenode(t_scenenode);
      };

  auto mbuild_evt_fcn =
      [this](std::unique_ptr<RenderEvents::create_mob> render_evt) {
        Ogre::Vector3 m_map_offsets{render_evt->m_map_offsets[0],
                                    render_evt->m_map_offsets[1],
                                    render_evt->m_map_offsets[2]};
        ...
        // (GameBGMap), but I formally would get it from a scene query.
        Ogre::AxisAlignedBox map_box = this->background->get_map_aab();
        this->place_mob(render_evt->model_id, render_evt->m_name, map_box,
                        m_map_offsets);
      };

  auto mobmove_evt_fcn =
      [this](std::unique_ptr<RenderEvents::move_mob> render_evt) {
        const double time_duration = render_evt->duration;
        ...
        if (mob_it != live_mobs.end()) {
          ...
          auto map_dimensions = map_box.getSize();
          auto parent_snode =
              scene_mgmt->getEntity(mob_name)->getParentSceneNode();
          Ogre::Vector3 movement{render_evt->delta[0], render_evt->delta[1],
                                 parent_snode->getPosition().z};
          // the position for the mob node to be at after the time_duration
          // elapses
          movement = map_dimensions * movement - map_box.getHalfSize();
  ...
  auto mobremove_evt_fcn =
      [this](std::unique_ptr<RenderEvents::remove_mob> render_evt) {
        auto mob_name = render_evt->name;
        ...
        // TODO: check if we are deleteing the correct scenenode. When we
        // execute this, we end up hanging forever at renderOneFrame down below,
        // so presumably something is getting nuked that shouldnt be
        auto m_scenenode =
            scene_mgmt->getEntity(mob_name)->getParentSceneNode();
        OgreUtil::nuke_scenenode(m_scenenode);
      };

  auto unitinfo_evt_fcn =
      [this](std::unique_ptr<RenderEvents::unit_information> info_evt) {
        // TODO: reeeeeally need to re-work how the GUI display will be arranged
        info_evt->base_stats = current_selection->getName();
        gui->display_information(info_evt->base_stats, info_evt->current_stats,
                                 info_evt->information);
      };

  auto gstate_transition_evt_fcn =
      [this](std::unique_ptr<RenderEvents::state_transition>
                 state_transition_evt) {
        // TODO: need to take action based on the state transition type -- i.e.
        // need to start a timer countdown if transitioning to IDLE, etc

        std::string old_state =
            state_transition_evt->old_state == GAME_STATE::IDLE
                ? "IDLE"
                : state_transition_evt->old_state == GAME_STATE::ACTIVE
                      ? "ACTIVE"
                      : "PAUSED";
        std::string new_state =
            state_transition_evt->new_state == GAME_STATE::IDLE
                ? "IDLE"
                : state_transition_evt->new_state == GAME_STATE::ACTIVE
                      ? "ACTIVE"
                      : "PAUSED";
        std::cout << "@FRONTEND: Transitioning State: " << old_state << " --> "
                  << new_state << std::endl;

        // NOTE: from here, we need to take various actions based on the state
        // transition being performed. We will probably want to break these out
        // into their own functions eventually
        if (state_transition_evt->old_state == GAME_STATE::ACTIVE &&
            state_transition_evt->new_state == GAME_STATE::IDLE) {
          // update the player inventory state with any new items obtained
          auto player_state =
              shared_gamestate_info->get_player_state_snapshot();
          gui->update_inventory_info(player_state);

          // TODO: add a countdown timer

        } else if (state_transition_evt->old_state == GAME_STATE::IDLE &&
                   state_transition_evt->new_state == GAME_STATE::ACTIVE) {

          // NOTE: what do we do if the inventory is open when the timer ends?
          // Do we auto/force close it? --> I guess so?
          ...
          if (ui_inventory_update_info.first) {
            // NOTE: since we were IDLE, any and all inventory updates should
            // come from the UI
            shared_gamestate_info->set_player_state_snapshot(
                std::move(ui_inventory_update_info.second));
  ...
  do {
    /*
            //render when triggered, or when the maximum allowable time has
       elapsed std::unique_lock<std::mutex> render_lk(render_mutx);
            render_cv.wait_for(render_lk, max_ms_perframe,
    ...
    */
    ...
    // how best to do this? check the input queues for messages? In this thread,
    // or in another one?
    handle_user_input();
    ...
    // NOTE: we probably only need to call this every few iterations (not every
    // iteration)
    update_gameinfo();
    ...
    std::chrono::duration<double, std::milli> time_duration(end_time -
                                                            start_time);
...
template <class BackendType> void OgreDisplay<BackendType>::setup_camera() {
  camera = scene_mgmt->createCamera("MinimalCamera");
  camera->setNearClipDistance(5);
  camera->setFarClipDistance(6000);
  // have a 4:3 aspect ratio, looking back along the Z-axis (should we do Y-axis
  // instead?)
...
template <class BackendType> bool OgreDisplay<BackendType>::ogre_setup() {
  // load resources
...
template <class BackendType> void OgreDisplay<BackendType>::gui_setup() {
...
template <class BackendType> void OgreDisplay<BackendType>::update_gameinfo() {
  ...
  // TODO: how do we merge changes made on the GUI-side to the master-copy of
  // the inventory?
}

template <class BackendType>
void OgreDisplay<BackendType>::dispatch_towerui_events() {
  // check if any tower modification events are pending, and if so, generate the
  // backend events to handle them
  if (gui->has_pending_tower_modifications()) {
    ...
    for (auto tmod : pending_mods) {
      ...
      std::tie(valid_tID, tower_row, tower_col) =
          tower_mapinfo.get_tower_location(tower_id);

      // notify the backend that a tower modification has to be performed
      if (valid_tID) {
        using tower_evt_t =
            UserTowerEvents::modify_tower_event<tower_properties, BackendType>;
        std::unique_ptr<UserTowerEvents::tower_event<BackendType>> td_evt =
            std::unique_ptr<tower_evt_t>(
                new tower_evt_t(tower_props, tower_row, tower_col));
...
template <class BackendType>
void OgreDisplay<BackendType>::get_mapcoords(
    const std::vector<float> &world_position, float &xnorm_coord,
    float &ynorm_coord, const Ogre::AxisAlignedBox &map_box) {
...
template <class BackendType>
void OgreDisplay<BackendType>::generate_information_request(
    std::vector<float> &&world_position) {
  float xnorm_coord, ynorm_coord;
  get_mapcoords(world_position, xnorm_coord, ynorm_coord,
                this->background->get_map_aab());

  using tower_evt_t = UserTowerEvents::print_tower_event<BackendType>;
  std::unique_ptr<UserTowerEvents::tower_event<BackendType>> td_evt =
      std::unique_ptr<tower_evt_t>(new tower_evt_t(ynorm_coord, xnorm_coord));
  td_event_queue->push(std::move(td_evt));
}

template <class BackendType>
void OgreDisplay<BackendType>::generate_tower(
    const float x_coord, const float y_coord, const float click_distance,
    const Ogre::AxisAlignedBox &map_box) {
  Ogre::Ray ray =
      camera->getCameraToViewportRay(x_coord / view_port->getActualWidth(),
                                     y_coord / view_port->getActualHeight());
  ...
  std::cout << world_click << " --> [" << norm_mapcoords_col << ", "
            << norm_mapcoords_row << "]" << std::endl;

  // this would come from the GUI (someday we'll have a GUI...)
  const int tier = 1;

  // make a unique ID for the tower being generated. This will be common to the
  // front and backend
  ...
  std::unique_ptr<UserTowerEvents::tower_event<BackendType>> td_evt =
      std::unique_ptr<tower_evt_t>(new tower_evt_t(
          tower_ID, tier, norm_mapcoords_row, norm_mapcoords_col));
  td_event_queue->push(std::move(td_evt));
}

template <class BackendType>
void OgreDisplay<BackendType>::place_tower(TowerModel *selected_tower,
                                           const uint32_t tower_ID,
                                           const std::string &tower_name,
                                           const Ogre::AxisAlignedBox &map_box,
                                           Ogre::Vector3 map_coord_offsets,
                                           Ogre::Vector3 world_coord_offsets) {
  std::cout << "TileCenter Offset: [" << map_coord_offsets.x << ", "
            << map_coord_offsets.y << "] --> ID: " << tower_ID << std::endl;

  // TODO: want the scale to be based on a few factors, such as the resolution,
  // map size, and fractal dimensions
  const float tower_scale = 1.0f / 4.0f;

  // store the tower IDs here
  tower_mapinfo.add_tower_ID(map_coord_offsets.x, map_coord_offsets.y,
                             tower_ID);

  auto tinfo = shared_gamestate_info->get_towerinfo(tower_ID);
  std::cout << tinfo.tower_name << std::endl;

  // NOTE: we want to have the tower ABOVE the map -- thus, its z coordinate has
  // to be non-zero
  const Ogre::Vector3 target_location{
      map_box.getHalfSize().x * (2 * (map_coord_offsets.x - 0.5f)),
  ...
  tower_obj->begin(tower_material_name,
                   Ogre::RenderOperation::OT_TRIANGLE_LIST);
  {
    for (const auto &polygon_pt : selected_tower->polygon_points_) {
      assert(polygon_pt.size() == 3);
      // NOTE: the position is set as (x,y,z), but the ordering of the data I
      // generate is (y,x,z). should probably change all my ordering to be
      // (x,y,z) to interoperate with Ogre better
    ...
    for (const auto &polygon_idx : selected_tower->polygon_mesh_)
      tower_obj->triangle(polygon_idx.at(0), polygon_idx.at(1),
                          polygon_idx.at(2));
  }
  tower_obj->end();
  ...
  tower_snode->setPosition(target_location.x, target_location.y,
                           target_location.z);
  ...
  */
  const std::vector<std::string> particle_types{"Tower/GreenGlow",
                                                "Tower/Sparks"};
  ...
  Ogre::ParticleSystem *particle_node =
      scene_mgmt->createParticleSystem(tower_name + "_particle", particle_type);
  // for testing -- see if we can properly modify the particlesystems
  if (particle_type == "Tower/GreenGlow") {
    particle_node->getEmitter(0)->setColour(
  ...
  Ogre::Animation *tower_animation =
      scene_mgmt->createAnimation(animation_id, duration);
  tower_animation->setInterpolationMode(Ogre::Animation::IM_SPLINE);
  Ogre::NodeAnimationTrack *tower_track =
      tower_animation->createNodeTrack(0, tower_snode);
  ...
  tower_keyf->setTranslate(
      Ogre::Vector3(target_location.x, target_location.y, target_location.z));
  tower_keyf->setScale(Ogre::Vector3(tower_scale, tower_scale, tower_scale));
  // t1
  tower_keyf = tower_track->createNodeKeyFrame(1.0f * step);
  tower_keyf->setTranslate(Ogre::Vector3(target_location.x, target_location.y,
                                         target_location.z + 5));
  tower_keyf->setScale(Ogre::Vector3(tower_scale, tower_scale, tower_scale));
  // etc...
...
template <class BackendType>
void OgreDisplay<BackendType>::place_mob(const CharacterModels::ModelIDs id,
                                         const std::string &mob_name,
                                         const Ogre::AxisAlignedBox &map_box,
                                         Ogre::Vector3 map_coord_offsets) {
  // NOTE: we want to have the tower ABOVE the map -- thus, its z coordinate has
  ...
  const Ogre::Vector3 target_location{
      map_box.getHalfSize().x * (2 * (map_coord_offsets.x - 0.5f)),
      map_box.getHalfSize().y * (2 * (map_coord_offsets.y - 0.5f)), 1};
  ...
  mob_snode->setPosition(target_location.x, target_location.y,
                         target_location.z);
...
void OgreDisplay<BackendType>::handle_user_input() {
  ...
  // for mouse dragging, we can move as roll, pitch, yaw based on the mouse
  // movement
  float cam_yaw = 0;
  float cam_pitch = 0;

  // take action based on the user input -- note: should we limit the user input
  // rate?
  ...
  while (input_events->pop(ui_evt)) {
    ...
    case ControllerUtil::INPUT_TYPE::D: {
      // have D mean deletion of the current object. This will likely change
      // once we add a real GUI...
      if (current_selection) {
        std::cout << "Deleting object " << current_selection->getName()
                  << std::endl;
    ...
    case ControllerUtil::INPUT_TYPE::C: {
      // if we have a tower selected, need to fire off an event to the GUI to
      // show the tower upgrade window
      if (current_selection) {
        // TODO: verify that a TOWER (not a monster) is selected

        // NOTE: we will need a lot more infrastructure here. In order to
        // upgrade the tower, we need to know the information about the tower,
        ...
        std::cout << "Combining " << current_selection->getName() << " @ "
                  << current_selection->getParentSceneNode()->getPosition()
                  << std::endl;

        auto selection_loc =
            current_selection->getParentSceneNode()->getPosition();
        std::vector<float> selection_position{selection_loc.x, selection_loc.y,
                                              selection_loc.z};
        float xnorm_coord, ynorm_coord;
        get_mapcoords(selection_position, xnorm_coord, ynorm_coord,
                      this->background->get_map_aab());
        auto selection_ID =
            tower_mapinfo.get_tower_ID(xnorm_coord, ynorm_coord);
    ...
    case ControllerUtil::INPUT_TYPE::LClick: {
      std::cout << "Mouse Lclick @[" << ui_evt.x_pos << ", " << ui_evt.y_pos
                << "]" << std::endl;

      // TODO: need to completely overhaul the raycasting and user selection
      // code. More re-computation than is necessary here

      // NOTE: current_selection is set to nullptr if nothing was selected.
      // Might want to disallow selecting the game map and certain other objects
      current_selection = view_detail::user_select(
          scene_mgmt, view_port, ui_evt.x_pos / width, ui_evt.y_pos / height);
      if (current_selection) {
        // gets the position clicked in the world -- can get the map coordinates
        // from this
        bool is_valid = false;
        float x_world_coord, y_world_coord, z_world_coord;
        std::tie(is_valid, x_world_coord, y_world_coord, z_world_coord) =
            view_detail::get_worldclick_coords(scene_mgmt, view_port,
                                               ui_evt.x_pos, ui_evt.y_pos);
        if (is_valid) {

          std::vector<float> worldclick_position{x_world_coord, y_world_coord,
                                                 z_world_coord};
          float xnorm_coord, ynorm_coord;
          get_mapcoords(worldclick_position, xnorm_coord, ynorm_coord,
                        this->background->get_map_aab());
          auto selection_towerID =
              tower_mapinfo.get_tower_ID(xnorm_coord, ynorm_coord);

          std::cout << "Selected tID " << selection_towerID << " -- "
                    << current_selection->getName() << " @ "
                    << current_selection->getParentSceneNode()->getPosition()
                    << std::endl;
          ...
          */
          ...
          // TODO: we should spawn an information request to the backend for the
          // selection and update the GUI accordingly request_selection = true;
        }
      }
      break;
    }
    case ControllerUtil::INPUT_TYPE::RClick: {
      std::cout << "Mouse Rclick @[" << ui_evt.x_pos << ", " << ui_evt.y_pos
                << "]" << std::endl;
      std::tie(valid_click, click_distance, obj) = view_detail::check_point(
          scene_mgmt, view_port, ui_evt.x_pos / width, ui_evt.y_pos / height);
      if (valid_click) {
        std::cout << "Total Dims: " << obj->getWorldBoundingBox() << std::endl;
        generate_tower(ui_evt.x_pos, ui_evt.y_pos, click_distance,
                       obj->getWorldBoundingBox());
      }
      break;
    }
    case ControllerUtil::INPUT_TYPE::MDrag: {
      // provides the difference in current mouse pos. from the previous mouse
      // pos. std::cout << "Mouse Drag @[" << ui_evt.x_pos << ", " <<
      // ui_evt.y_pos << "]" << std::endl;
...
bool OgreDisplay<BackendType>::frameRenderingQueued(
  ...
  // a seperate frame listener (i.e. in the controller? or in a GUI-specific
  // frame listener?
